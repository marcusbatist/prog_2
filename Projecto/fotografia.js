document.addEventListener("DOMContentLoaded", () => {
  const carouselContainer = document.getElementById("fotografia-carousel");
  const prevButton = document.getElementById("fotografia-prevBtn");
  const nextButton = document.getElementById("fotografia-nextBtn");

  let currentIndex = 0;

  fetch("data.json")
    .then((response) => response.json())
    .then((data) => data.filter((element) => element.type === "Fotografia"))
    .then((data) => {
